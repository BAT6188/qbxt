...
public class BaseDaoImpl<T> extends HibernateDaoSupport implements
	...
	public void save(List<T> entityObjectList) throws Exception {
		logger.debug("保存"+entityObjectList.size()+"条数据到数据库");
		//每10条
		int pageSize=10;
		//总数量
		int totalCount=entityObjectList.size();
		//共多少页
		int pageCount=(totalCount+pageSize-1)/pageSize;
		Session session=getHibernateTemplate().getSessionFactory().getCurrentSession();
		for (int i = 0; i < totalCount; i++) {
			getHibernateTemplate().save(entityObjectList.get(i));
			if(i%10==0){
				//清空缓存
				session.flush();
				session.clear();
	...
	@SuppressWarnings("unchecked")
	public Object getRows(final String sql)throws Exception {
		logger.debug("SQL语句查询(" + sql + ").");
	...
	@SuppressWarnings("unchecked")
	public List findBySqlAnPersonStore(final String sql,final Class<PersonStore> entityClass, final int sizePage, final int startRecord)
			throws Exception {
		logger.debug("SQL语句查询(" + sql + ").");
		return (List) getHibernateTemplate().execute(new HibernateCallback<Object>() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
