 ...
 * 显示业务文档附件
 */
Ext.define('Ushine.docInfo.ServiceDocStoreFileGridPanel', {
	extend: 'Ext.grid.Panel',
	//enableColumnResize:是否允许改变列宽，默认为true
	id:"ServiceDocStoreFileGridPanel",
	...
	constructor: function(sjnum,record) {
		...
		this.store=Ext.create('Ext.data.JsonStore',{
			...
			proxy:{
				type:'ajax',
				url:'getSericeDocStoreTempFile.do?number='+sjnum+'&appendix='+attaches,
				method:'get',
				simpleSortMode:true,
				reader:{
	                type: 'json',
		    ...
		    {text: '操作',sortable: false,width:130,flex: 3,xtype:'actioncolumn',
		    	items:[{
		            ...
		            handler: function(grid, rowIndex, colIndex){
		            	...
		            	console.log(data);
		            	var index=data.filePath.indexOf("tempServiceDocStoreFileUpload_");
		            	if(index>-1){
		            		//代表临时上传的
		            		//后台删掉
		            		var fileName=data.filePath.substring("tempServiceDocStoreFileUpload_".length,data.filePath.length);
		            		console.log(fileName);
		            		Ext.Ajax.request({
			            	    url: 'deleteServiceDocStoreFile.do',
			            	    params: {
			            	        fileName: fileName,
			            	        number:sjnum
			            	    },
