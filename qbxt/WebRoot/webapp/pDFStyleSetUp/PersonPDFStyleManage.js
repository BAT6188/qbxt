 ...
 * 人员库PDF设置penal
 */
Ext.define('Ushine.pDFStyleSetUp.PersonPDFStyleManage',{
	extend:'Ext.panel.Panel',
	id:'personinPDFStylefomanage-info',
	title:'人员库PDF样式设置',
	...
	constructor: function(url) {
		...
		this.items = [{
			...
			items:{
					...
					items:[
					       Ext.create('Ushine.buttons.IconButton', {
					        ...
					        Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function () {
					    		   Ext.Ajax.request({
			    					   url: 'restPersonStorePDFStyle.do',
			    					   success: function(response){
			    						   var text = response.responseText;
			    						   var obj=Ext.JSON.decode(text);
			    						   Ext.create('Ushine.utils.Msg').onInfo(obj.msg);
			    						   //刷新页面
											...
											Ext.getCmp('content_frame').add(new Ushine.pDFStyleSetUp.PDFStyleSetUp('Ushine.pDFStyleSetUp.PersonPDFStyleManage',url));
			    					   },
			    					   failure: function(form, action) {
			    						   Ext.create('Ushine.utils.Msg').onInfo('获取当前样式失败，请联系管理员');
			    					   }
					       ...
					       Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function () {
					    		 //两次编码
					    		location.href = "downloadPersonStorePDFTemplateFile.do";
					    	   }
					       })
					    ]}]
				}
				...
				//加载图片
				items:[
					self.getPersonStorePDFTemplate()
				]
	...
	getPersonStorePDFTemplate:function(){
		var self=this;
		...
		Ext.create('Ext.data.Store',{
			...
			id:'personStorePDFTemplate',
			proxy:{
				type:'ajax',
				url:'getPersonStorePDFTemplate.do',
				reader:{
					type:"json"
				}
			},
		...
		var com=Ext.create('Ext.view.View', {
		    store: Ext.data.StoreManager.lookup('personStorePDFTemplate'),
		    tpl: imageTpl,
		    //必须的
		    itemSelector: 'img',
			listeners: {
		       		     ...
		       		     // 隐藏表单操作
		   					Ext.create('Ushine.buttons.MiniButton', {
		   						...
		   						handler: function () {
		   							...
		   							var personPdfSet = Ext.getCmp('personPdfSet');
		   							if(from.hidden){
		   								this.removeCls("hideBtn");
		   								this.addCls("displayBtn");
		   								from.show();
		   								from2.show();
		   								Ext.getCmp('personPdfSet').setWidth(personPdfSet.width);
		   								Ext.getCmp('personPdfSet').setHeight(personPdfSet.height+63);
		   							}else{
			   					...
			   					Ext.create('Ushine.buttons.MiniButton', {
			   						...
			   						handler: function () {
			   							...
			   							if(from.hidden){
			   								...
			   								from.show();
			   								Ext.getCmp('personPdfSet').setWidth(personPdfSet.width);
			   								Ext.getCmp('personPdfSet').setHeight(personPdfSet.height+33);
			   							}else{
			   								this.removeCls("displayBtn");
			   								this.addCls("hideBtn");
			   								from.hide();
