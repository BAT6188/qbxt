 ...
 *组织库PDF样式设置panel
 */
Ext.define('Ushine.pDFStyleSetUp.OrganizPDFStyleManage',{
	extend:'Ext.panel.Panel',
	id:'organizPDFStylefomanage-info',
	title:'组织库PDF样式设置',
	...
	constructor: function(url) {
		...
		this.items = [{
			...
			items:{
		    	   ...
		    	   layout: "column", //行1
					...
					items:[
					       Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function() {
					    		   Ext.Ajax.request({
			    					   url: 'getOrganizStorePDFStyle.do',
					        ...
					        Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function () {
					    		   Ext.Ajax.request({
			    					   url: 'restOrganizStorePDFStyle.do',
			    					   success: function(response){
			    						   var text = response.responseText;
			    						   var obj=Ext.JSON.decode(text);
			    						   Ext.create('Ushine.utils.Msg').onInfo(obj.msg);
			    						   //刷新页面
											...
											Ext.getCmp('content_frame').add(new Ushine.pDFStyleSetUp.PDFStyleSetUp('Ushine.pDFStyleSetUp.OrganizPDFStyleManage',url));
			    					   },
			    					   failure: function(form, action) {
			    						   Ext.create('Ushine.utils.Msg').onInfo('获取当前样式失败，请联系管理员');
			    					   }
					       ...
					       Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function () {
					    		 //两次编码
					    		location.href = "downloadOrganizStorePDFTemplateFile.do";
					    	   }
					       })
					    ]}]
				}
				...
				//加载图片
				items:[
					self.getOrganizStorePDFTemplate()
				]
	...
	getOrganizStorePDFTemplate:function(){
		var self=this;
		Ext.define('OrganizPhotoModel',{
			extend:'Ext.data.Model',
			fields:['src']
		});
		Ext.create('Ext.data.Store',{
			model:'OrganizPhotoModel',
			id:'organizStorePDFTemplate',
			proxy:{
				type:'ajax',
				url:'getOrganizStorePDFTemplate.do',
				reader:{
					type:"json"
				}
			},
		...
		var com=Ext.create('Ext.view.View', {
		    store: Ext.data.StoreManager.lookup('organizStorePDFTemplate'),
		    tpl: imageTpl,
		    //必须的
		    itemSelector: 'img',
			listeners: {
			...
			constructor:function(){
		       		     ...
		       		     // 隐藏表单操作
		   					Ext.create('Ushine.buttons.MiniButton', {
		   						...
		   						handler: function () {
		   							...
		   							var organizPdfSet = Ext.getCmp('organizPdfSet');
		   							if(from.hidden){
		   								this.removeCls("hideBtn");
		   								this.addCls("displayBtn");
		   								from.show();
		   								Ext.getCmp('organizPdfSet').setWidth(organizPdfSet.width);
		   								Ext.getCmp('organizPdfSet').setHeight(organizPdfSet.height+63);
		   							}else{
			   					...
			   					Ext.create('Ushine.buttons.MiniButton', {
			   						...
			   						handler: function () {
			   							...
			   							if(from.hidden){
			   								...
			   								from.show();
			   								Ext.getCmp('organizPdfSet').setWidth(organizPdfSet.width);
			   								Ext.getCmp('organizPdfSet').setHeight(organizPdfSet.height+33);
			   							}else{
			   								this.removeCls("displayBtn");
			   								this.addCls("hideBtn");
			   								from.hide();
				...
				this.buttons=[
				  	  		Ext.create('Ushine.buttons.Button', {
				  	   			...
				  	   			handler:function(){
				  	   			var from = Ext.getCmp('personSetUpPDFInfoForm');
				  	   			if(from.getForm().isValid()){
					   					from.getForm().submit({
					   						url:'setUpOrganizPDFStyle.do',
											method:'POST',
											waitMsg:'字体设置中',
											success:function(form,action){
												var obj=Ext.JSON.decode(action.response.responseText);
					   							...
					   							Ext.getCmp('content_frame').removeAll();
												Ext.getCmp('content_frame').add(new Ushine.pDFStyleSetUp.PDFStyleSetUp('Ushine.pDFStyleSetUp.OrganizPDFStyleManage',url));
					   							Ext.getCmp('organizPdfSet').close();
											},
											// 提交失败的回调函数
											failure : function() {
												Ext.create('Ushine.utils.Msg').onInfo("服务器出现错误请稍后再试!");
