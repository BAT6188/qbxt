 ...
 * 媒体刊物库PDF样式设置panel
 */
Ext.define('Ushine.pDFStyleSetUp.WebsiteJounalPDFStyleManage',{
	extend:'Ext.panel.Panel',
	id:'websiteJounalPDFStylefomanage-info',
	title:'媒体刊物库PDF样式设置',
	...
	constructor: function(url) {
		...
		this.items = [{
			...
			items:{
		    	   ...
		    	   layout: "column", //行1
					...
					items:[
					       Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function() {
					    		   Ext.Ajax.request({
			    					   url: 'getWebsiteJounalStorePDFStyle.do',
					        ...
					        Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function () {
					    		   Ext.Ajax.request({
			    					   url: 'restWebsiteJounalStorePDFStyle.do',
			    					   success: function(response){
			    						   var text = response.responseText;
			    						   var obj=Ext.JSON.decode(text);
			    						   Ext.create('Ushine.utils.Msg').onInfo(obj.msg);
			    						   //刷新页面
											...
											Ext.getCmp('content_frame').add(new Ushine.pDFStyleSetUp.PDFStyleSetUp('Ushine.pDFStyleSetUp.WebsiteJounalPDFStyleManage',url));
			    					   },
			    					   failure: function(form, action) {
			    						   Ext.create('Ushine.utils.Msg').onInfo('获取当前样式失败，请联系管理员');
			    					   }
					       ...
					       Ext.create('Ushine.buttons.IconButton', {
					    	   ...
					    	   handler: function () {
					    		 //两次编码
					    		location.href = "downloadWebsiteJounalStorePDFTemplateFile.do";
					    	   }
					       })
					    ]}]
				}
				...
				margin:'10 5 10 5',
				flex:1,
				id:'websiteJournalStoreTemplateImage',
				...
				//加载图片
				items:[
					self.getWebsiteJournalStorePDFTemplate()
				]
	...
	getWebsiteJournalStorePDFTemplate:function(){
		var self=this;
		Ext.define('WebsiteJounalPhotoModel',{
			extend:'Ext.data.Model',
			fields:['src']
		});
		Ext.create('Ext.data.Store',{
			model:'WebsiteJounalPhotoModel',
			id:'websiteJounalStorePDFTemplate',
			proxy:{
				type:'ajax',
				url:'getWebsiteJournalStorePDFTemplate.do',
				reader:{
					type:"json"
				}
			},
		...
		var com=Ext.create('Ext.view.View', {
		    store: Ext.data.StoreManager.lookup('websiteJounalStorePDFTemplate'),
		    tpl: imageTpl,
		    //必须的
		    itemSelector: 'img',
			listeners: {
	...
	setUpPDFStyle:function(obj,url){
		Ext.define('setUpPDFStyleInfoWin',{
			...
			title : "媒体刊物库PDF样式设置", // 标题
			width : 766, // 宽度
			height: 370,
			id:'websiteJounalPdfSet',
			autoHeight : true, // 宽度
			collapsible : true, // 是否可以伸缩
			plain : true,
			closable : true,
		...
		//	iconCls : "houfei-addicon", // 图标样式
			...
			constructor:function(){
				...
				this.items=[{
					id:'websiteJounalSetUpPDFInfoForm',
					xtype:'form',
					border:false,
					layout:'vbox',
					items:[{
		       		     ...
		       		     // 隐藏表单操作
		   					Ext.create('Ushine.buttons.MiniButton', {
		   						...
		   						handler: function () {
		   							...
		   							var websiteJounalPdfSet = Ext.getCmp('websiteJounalPdfSet');
		   							if(from.hidden){
		   								this.removeCls("hideBtn");
		   								this.addCls("displayBtn");
		   								from.show();
		   								Ext.getCmp('websiteJounalPdfSet').setWidth(websiteJounalPdfSet.width);
		   								Ext.getCmp('websiteJounalPdfSet').setHeight(websiteJounalPdfSet.height+63);
		   							}else{
			   					...
			   					Ext.create('Ushine.buttons.MiniButton', {
			   						...
			   						handler: function () {
			   							...
			   							if(from.hidden){
			   								...
			   								from.show();
			   								Ext.getCmp('websiteJounalPdfSet').setWidth(websiteJounalPdfSet.width);
			   								Ext.getCmp('websiteJounalPdfSet').setHeight(websiteJounalPdfSet.height+33);
			   							}else{
			   								this.removeCls("displayBtn");
			   								this.addCls("hideBtn");
			   								from.hide();
				...
				this.buttons=[
				  	  		Ext.create('Ushine.buttons.Button', {
				  	   			...
				  	   			handler:function(){
				  	   			var from = Ext.getCmp('websiteJounalSetUpPDFInfoForm');
				  	   			if(from.getForm().isValid()){
					   					from.getForm().submit({
					   						url:'setUpWebsiteJounalPDFStyle.do',
											method:'POST',
											waitMsg:'字体设置中',
											success:function(form,action){
												var obj=Ext.JSON.decode(action.response.responseText);
					   							...
					   							Ext.getCmp('content_frame').removeAll();
												Ext.getCmp('content_frame').add(new Ushine.pDFStyleSetUp.PDFStyleSetUp('Ushine.pDFStyleSetUp.WebsiteJounalPDFStyleManage',url));
					   							Ext.getCmp('websiteJounalPdfSet').close();
											},
											// 提交失败的回调函数
											failure : function() {
												Ext.create('Ushine.utils.Msg').onInfo("服务器出现错误请稍后再试!");
