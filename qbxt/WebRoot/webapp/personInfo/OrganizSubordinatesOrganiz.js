/**
 * 组织下属组织 author：donghao
 */
Ext.define('Ushine.personInfo.OrganizSubordinatesOrganiz', {
	extend : 'Ext.panel.Panel',
	id : 'organizSubordinates_panel',
	region : 'center',
	...
	constructor : function(organizId) {
			...
			items : {
				...
				items : [{
					...
					items : [Ext.create('Ushine.buttons.IconButton', {
										...
										handler : function() {
											//弹出新增人员window
								    		   self.selectAssociatedOrganizData(organizId);
										}
									}),
								...
								// width:120,
								handler : function() {
									var organizSubordinatesGrid=Ext.getCmp('organizSubordinatesGrid');					    	   	
								   	   if(organizSubordinatesGrid.getSelectionModel().hasSelection()){
								   	   		...
								   	   		Ext.Msg.confirm("提示","确定要解除选中的组织吗?",function(btn){
								   	   			if (btn == 'yes') {
								   	   			   self.removeOrganizSubOrganiz(ids,organizId);
						    				   }
								   	   		})
								   	   }else{
					...
					items : [{
								...
								value : new Date()
										...
										handler : function() {
											// 查询
											self.findSubordinatesOrganiz(organizId);
										}
									}), Ext.create('Ushine.buttons.Button', {
										text : '条件重置',
										width : 100,
		...
		new Ushine.personInfo.OrganizSubordinatesOrganizInfoGridPanel(organizId)];
		this.callParent();
	},
	// 查询
	findSubordinatesOrganiz : function(organizId) {
		this.remove('oranizSubordinatesOrganiz_grid');
		this.add(new Ushine.personInfo.OrganizSubordinatesOrganizInfoGridPanel(organizId));
	},
	//选择关联组织数据
	selectAssociatedOrganizData:function(organizId){
				...
				this.buttons=[
				      	  	Ext.create('Ushine.buttons.Button', {
				      	   		...
				      	   		handler: function () {
				      	   		 ...
				      	   		 //ajax操作关联人员
				      	   		 Ext.Ajax.request({
		    						    url: 'associatedOrganizSubOrganiz.do',
		    						    actionMethods: {
		    				                create : 'POST',
		    						    ...
		    						    success: function(response){
		    							     ...
		    							     if(obj.status){
		    							    	 ...
		    							    	 Ext.getCmp('organizSubordinates_panel').findSubordinatesOrganiz(organizId);
	...
	//解除线索组织关系
	removeOrganizSubOrganiz:function(ids,organizId){
		var self=this;
		Ext.Ajax.request({
		    url: 'removeOrganizSubOrganiz.do',
		    method:'POST',
		    params: {
		    	ids: ids
		    },
		    success: function(response, opts){
		       ...
		       if(obj.success){
		    	   ...
		    	   Ext.Msg.alert('提示',"解除下属组织失败，请联系管理员！");
		       }
		    },
		    failure:function(){
		    	 Ext.Msg.alert('提示',"解除下属组织失败，请求后台服务失败！");
		    }
		});
	}
