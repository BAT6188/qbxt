 ...
 * 人员上级组织
 */
Ext.define('Ushine.personInfo.PersonAtHigherLevelOrganizeInfoManage',{
	...
	constructor: function(personStoreId) {
		var self = this;
			...
			items:{
				...
				items:[{
		    	   layout: "column", //行1
					...
					items:[Ext.create('Ushine.buttons.IconButton', {
						...
						handler : function() {
							//弹出选择组织窗体
				    		self.selectAssociatedOrganizData(personStoreId);
						}
					}),
				...
				handler : function() {
					var organizSubordinatesGrid=Ext.getCmp('organizeInfoGridPanel');					    	   	
				   	   if(organizSubordinatesGrid.getSelectionModel().hasSelection()){
				   	   		...
				   	   		Ext.Msg.confirm("提示","确定要解除选中的组织吗?",function(btn){
				   	   			if (btn == 'yes') {
				   	   			   self.removeOrgSubordinatesPeson(ids,personStoreId);
		    				   }
				   	   		})
				   	   }else{
					...
					Ext.create('Ushine.buttons.Button', {
			        	...
			        	handler:function(){
			        		//查询
	                        self.findSubordinatesOrganiz(personStoreId);
		                }
			        }),Ext.create('Ushine.buttons.Button', {
			        	text : '条件重置',
			        	width:100,
		...
		//上级组织gridpanel
		new Ushine.personInfo.PersonAtHigherLevelOrganizeInfoGridPanel(personStoreId)
		];	
		this.callParent();		
	},//选择关联人员数据
	selectAssociatedOrganizData:function(personStoreId){
				...
				this.buttons=[
				      	  	Ext.create('Ushine.buttons.Button', {
				      	   		...
				      	   		handler: function () {
				      	   		 ...
				      	   		 //ajax操作关联人员
				      	   		 Ext.Ajax.request({
		    						    url: 'associatedPersonSubOrganiz.do',
		    						    actionMethods: {
		    				                create : 'POST',
		    						    ...
		    						    success: function(response){
		    							     ...
		    							     if(obj.status){
		    							    	 ...
		    							    	 Ext.getCmp('organizeinfomanage-panel').findSubordinatesOrganiz(personStoreId);
	...
	findSubordinatesOrganiz : function(personStoreId) {
		this.remove('p_organizeinfo_grid');
		this.add(new Ushine.personInfo.PersonAtHigherLevelOrganizeInfoGridPanel(personStoreId));
	},
	//解除人员组织关系
	removeOrgSubordinatesPeson:function(ids,personStoreId){
		var self=this;
		Ext.Ajax.request({
		    ...
		    success: function(response, opts){
		       ...
		       if(obj.success){
		    	   ...
		    	   Ext.Msg.alert('提示',"解除人员上级组织失败，请联系管理员！");
		       }
		    },
		    failure:function(){
		    	 Ext.Msg.alert('提示',"解除人员上级组织失败，请求后台服务失败！");
		    }
		});
	}
});
