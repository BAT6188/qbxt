 ...
 * @author 王百林
 */
Ext.define('Ushine.utils.PersonStoreFileGridPanel', {
	extend: 'Ext.grid.Panel',
	//enableColumnResize:是否允许改变列宽，默认为true
	id:"personStoreFileGridPanelId",
	...
	constructor: function(sjnum,record) {
		...
		this.store=Ext.create('Ext.data.JsonStore',{
			...
			proxy:{
				type:'ajax',
				url:'findPersonStoreTempFile.do?number='+sjnum+'&appendix='+appendix,
				method:'POST',
				simpleSortMode:true,
				reader:{
	                type: 'json',
		    ...
		    {text: '操作',sortable: false,width:130,flex: 3,xtype:'actioncolumn',
		    	items:[{
		            ...
		            handler: function(grid, rowIndex, colIndex){
		            	...
		            	//console.log(data);
		            	var index=data.filePath.indexOf("tempPersonStoreFileUpload_");
		            	if(index>-1){
		            		//代表临时上传的
		            		//后台删掉
		            		var fileName=data.filePath.substring("tempPersonStoreFileUpload_".length,data.filePath.length);
		            		console.log(fileName);
		            		Ext.Ajax.request({
			            	    url: 'deletePersonStoreFile.do',
			            	    params: {
			            	        fileName: fileName,
			            	        number:sjnum
			            	    },
