...
public class WebsiteJournalStoreServiceImplTest {
	...
	/**
	 * 添加每条索引对象记录
	 * @param list WebsiteJournalStore集合
	 * @return
	 */
	private static List<Document> addDocs(List<WebsiteJournalStore> storeList){
		List<Document> documents=new ArrayList<Document>();
		for (WebsiteJournalStore store : storeList) {
			Document document=new Document();
			//添加索引
			document.add(new Field("id", store.getId(), Store.YES, Index.NO));
			//名称
			if(null!=store.getName()){
				//不为空
				document.add(new Field("name", store.getName(), Store.YES, Index.ANALYZED));
			}else{
				document.add(new Field("name", "", Store.YES, Index.NO));
			}
			//网址
			if(null!=store.getWebsiteURL()){
				...
				document.add(new Field("websiteURL", "", Store.YES, Index.NO));
			}
			//服务器地址
			if(null!=store.getServerAddress()){
				...
				document.add(new Field("serverAddress", "", Store.YES, Index.NO));
			}
			//创办地址
			if(null!=store.getEstablishAddress()){
				...
				document.add(new Field("establishAddress", "", Store.YES, Index.NO));
			}
			//主要发行地
			if(null!=store.getMainWholesaleAddress()){
				...
				document.add(new Field("mainWholesaleAddress", "", Store.YES, Index.NO));
			}
			//创办人
			if(null!=store.getEstablishPerson()){
				...
				document.add(new Field("establishPerson", "", Store.YES, Index.NO));
			}
			//创办时间
			if(null!=store.getEstablishTime()){
				//不为空
				document.add(new Field("establishTime", store.getEstablishTime(), Store.YES, Index.ANALYZED));
			}else{
				document.add(new Field("establishTime", "", Store.YES, Index.NO));
			}
			//基本情况
			if(null!=store.getBasicCondition()){
				//不为空
				document.add(new Field("basicCondition", store.getBasicCondition(), Store.NO, Index.ANALYZED));
			}else{
				document.add(new Field("basicCondition", "", Store.YES, Index.NO));
			}
			document.add(new Field("infoType", store.getInfoType().getTypeName(), Store.YES, Index.ANALYZED));
			document.add(new Field("uid", store.getUid(), Store.YES, Index.NO));
			document.add(new Field("oid", store.getOid(), Store.YES, Index.NO));
			document.add(new Field("did", store.getDid(), Store.YES, Index.NO));
			//document.add(new Field("createDate", store.getCreateDate(), Store.YES, Index.ANALYZED));
